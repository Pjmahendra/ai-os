...
export async function runScheduledAutomations() {
  // Cheap first gate: skip the DB round-trip entirely when this same
  // process is already mid-run. The real cross-instance guard is the
  // Postgres advisory lock in withSchedulerLock.
  if (schedulerRunning) {
    console.log(
  ...
  try {
    await withSchedulerLock(runScheduledAutomationsBody);
  } finally {
    schedulerRunning = false;
  }
}

async function runScheduledAutomationsBody(): Promise<void> {
  ...
  for (const automation of automations) {
    try {
      ...
      const previousRun = expression.prev().toDate();
      const previousRunTime = previousRun.getTime();
      const lastRunTime = automation.lastRunAt?.getTime() ?? 0;
